...
class UserDaoSpec extends Specification {
  ...
  "UserDao" should {
    "Add user" in new WithApplication(Statics.fakeApp){
    ...
    "Delete added user" in new WithApplication(Statics.fakeApp){
      Users.add(user).map(r => r must equalTo(1))
      Users.delete(1).map(r => r must equalTo(1))
    }

    "Not delete invalid user" in new WithApplication(Statics.fakeApp){
      Users.delete(1).map(r => r must equalTo(0))
    }

    "Update user" in new WithApplication(Statics.fakeApp){
      Users.add(user).map(r => r must equalTo(1))
      Users.updateUser(1, user).map(r => r must equalTo(1))
    }

    "Not update invalid user" in new WithApplication(Statics.fakeApp){
      Users.updateUser(1, null).map(r => r must equalTo(0))
    }

    "Block user" in new WithApplication(Statics.fakeApp){
      Users.add(user).map(r => r must equalTo(1))
      Users.setBlock(1, true).map(r => r must equalTo(1))
    }

    "Not block invalid user" in new WithApplication(Statics.fakeApp){
    ...
    "Get by email" in new WithApplication(Statics.fakeApp){
      Users.add(user).map(r => r must equalTo(1))
      Users.getFromEmail(user.Email).map(r => r must equalTo(Some))
    }

    "Get by email invalid" in new WithApplication(Statics.fakeApp){
      Users.getFromEmail(user.Email).map(r => r must equalTo(None))
    }

    "Get by apikey" in new WithApplication(Statics.fakeApp){
      Users.add(user).map(r => r must equalTo(1))
      Users.getFromApiKey(user.ApiKey).map(r => r must equalTo(Some))
    }

    "Get by apikey invalid" in new WithApplication(Statics.fakeApp){
